    ...
    for (unsigned int i = 0; i < platforms.size(); i++) {
        ...
        for (unsigned int j = 0; j < devices.size(); j++) {
            sstream << "\n      Device " << j << ", " << devices[j].getInfo<CL_DEVICE_NAME>()
                    << ", version: " << devices[j].getInfo<CL_DEVICE_VERSION>();
...
cl::Context GetContext(int platform_id, int device_id) {
    ...
    for (unsigned int i = 0; i < platforms.size(); i++) {
        ...
        for (unsigned int j = 0; j < devices.size(); j++) {
            if ((i == platform_id) && (j == device_id)) return cl::Context({devices[j]});
        }
    }
...
string GetFullProfilingInfo(const cl::Event& evnt, ProfilingResolution resolution) {
    ...
    sstream << "Queued "
            << (evnt.getProfilingInfo<CL_PROFILING_COMMAND_SUBMIT>() -
                evnt.getProfilingInfo<CL_PROFILING_COMMAND_QUEUED>()) /
                       resolution;
    sstream << ", Submitted "
